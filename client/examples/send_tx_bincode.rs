...
//!
//! options:
//!   --server       land server address (default: 127.0.0.1:8080)
//!   --rpc          solana rpc url (default: http://127.0.0.1:8899)
//!   --keypair      path to keypair json file (optional, creates ephemeral if not provided)
//!   --fanout       number of leaders to send to (default: 1)
//!   --recipient    recipient pubkey (optional, sends to self if not provided)
...
async fn main() {
    ...
    let mut land = match Land::connect(&server) {
        Ok(c) => c,
        Err(e) => {
